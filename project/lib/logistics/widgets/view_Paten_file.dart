...
class ViewPatenFileLogistic extends StatefulWidget {
  const ViewPatenFileLogistic({
    Key? key,
  ...
  @override
  State<ViewPatenFileLogistic> createState() => _ViewPatenFileState();
}

class _ViewPatenFileState extends State<ViewPatenFileLogistic> {
  /// PDF view handle
      ...
      onTap: () async {
        ...
        if ((widget.patenFileUrl != '' && widget.patenFileUrl != null) || (widget.patenName != null && widget.patenBytes != null)) {
          ...
          if (patenFileExtension != 'pdf') {
            ...
            File? localFile;
            if (widget.patenFileUrl == null) {
              localFile = context.read<PurchaseServiceLogistic>().localFile;
            ...
            final File file = widget.isAsset
                ? await context.read<PurchaseServiceLogistic>().loadAsset(
                      widget.patenName!,
                      widget.patenBytes!,
                    )
                : await context.read<PurchaseServiceLogistic>().loadNetwork(
                      widget.patenFileUrl!,
                    );
            if (!mounted) return;
            await showDialog(
